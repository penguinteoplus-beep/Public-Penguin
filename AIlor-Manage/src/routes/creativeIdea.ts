...
// 注意：特定路由必须放在 /:id 动态路由之前，否则会被抢先匹配
...
router.post('/', (req: Request, res: Response<ApiResponse<CreativeIdea>>) => {
  try {
    const { title, prompt, imageUrl, isSmart, isSmartPlus, isBP, smartPlusConfig, bpFields, cost, suggestedAspectRatio, suggestedResolution, order } = req.body;
